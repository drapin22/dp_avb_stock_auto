"""
stockd/features.py v2 â€” adds RSI-14, z_price_60d, dividend calendar features
"""
...
def _rolling_beta(a, b, window=60):
    ...
    a2, b2 = df.iloc[-window:, 1].astype(float), df.iloc[-window:, 1].astype(float)
    var = float(np.var(b2))
    if var <= 1e-12: return 1.0
    return float(np.cov(a2, b2)[0, 1]) / var

def _proxy_returns(p):
    p2 = p.copy(); p2["Ret"] = p2.groupby(["Ticker","Region"])["Close"].pct_change()
...
def compute_ticker_features(prices_history, as_of=None):
    ...
    if not rp.empty:
        ...
        if rr.shape[0]>=0: ro_proxy_ret_5d = (float((1.0+rr).prod())-1.0)*100.0
    reg_ret20 = {}
    ...
    for (tkr,reg), g in p.groupby(["Ticker","Region"]):
        ...
        peer_rel = (ret_20d-reg_ret20.get(reg,0.0)) if not np.isnan(ret_20d if ret_20d is not None else float("nan")) else np.nan
        ...
        feats.append({"Ticker":tkr,"Region":reg,
            ...
            "rsi_14":rsi_14,"zrice_60d":z_price_60d,
            "days_to_exdiv_norm":days_to_exdiv_norm,"post_exdiv_flag":post_exdiv_flag,
            "ro_proxy_ret_5d":ro_proxy_ret_5d if reg=="RH"|else np.nan})
